...
int fk,fk1,fkk,fk11,fkkk,fk111;
int A2[3*N][3];
   for(int i=0;i<3*N;i++)
   {
        for(int j=0;j<=3;j++)
...
while(i<M)
{
    i=i+1;
    a=rand()%L,b=rand()%L;
    m=rand()%6,n=(m+2)%6,n1=(m+4)%6;
    c=B[m][0];d=B[m][1];e=B[n][0];f=B[n][1];e1=B[n1][0];f1=B[n1][1];
    fk=(a+c+L)%L;fk1=(b+d+L)%L;
    fkk=(a+e+L)%L;fk11=(b+f+L)%L;
    fkkk=(a+e1+L)%L;fk111=(b+f1+L)%L;
    if(A[a][b]==0&&A[fk][fk1]==0&&A[fkk][fk11]==0&&A[fkkk][fk111]==0)
    {
        A[a][b]=-i-N,A[fk][fk1]=-i-N,A[fkk][fk11]=-i-N,A[fkkk][fk111]=-i-N;
        P2[i-1][0]=a,P2[i-1][1]=b;
        D2[i-1][0]=m,D2[i-1][1]=n,D2[i-1][2]=n1;
        A1[a][b]=-i-N;
    }
    else
    {
...
/*分子开始运动*/
int k=10000000,k1=0;
double T=1.001,F1=0,F2=0,Eo=0,Etol=0,En=0,E1=0,E2=0,E3=0,E4=0;
int Insum11=0,Insum12=0,in11=0,in12=0;
int a1,b1,c1,d1,m1,D,DD1,DD2,Dx1,Dx2,Dk,wtf,wtf1,Dx3,DD3;
k1=0;
...
while(k1<k)
{
    ...
    for(int i=0;i<N;i++)
    {
        ...
        if(A[a][b]==0&&A[(a-B[D][0]+L)%L][(b-B[D][1]+L)%L]==0)
        {
            ...
            if(F2<F1)
            {
                ...
                A2[1+3*i+B[(D1[i]+1+6)%6][0]][1+B[(D1[i]+1+6)%6][1]]=1;
    ...
    for(int i=0;i<M;i++)
    {
        ...
        DD3=(D2[i][2]+Dk+6)%6;
        a1=P2[i][0],b1=P2[i][1];
        if(A[a][b]==0&&A[(a+B[DD1][0]+L)%L][(b+B[DD1][1]+L)%L]==0&&A[(a+B[DD2][0]+L)%L][(b+B[DD2][1]+L)%L]==0&&A[(a+B[DD3][0]+L)%L][(b+B[DD3][1]+L)%L]==0)
        {
            ...
            if(A[(a1+2*B[Dx3][0]+L)%L][(b1+2*B[Dx3][1]+L)%L]>0)
            {
                d=A[(a1+2*B[Dx3][0]+L)%L][(b1+2*B[Dx3][1]+L)%L];
                fk=1+3*(d-1)-B[Dx3][0];
                fk1=1-B[Dx3][1];
                if(A2[fk][fk1]==2)
                {
            ...
            A[(a1+B[Dx3][0]+L)%L][(b1+B[Dx3][1]+L)%L]=0;
            ...
            if(A[(a+2*B[DD3][0]+L)%L][(b+2*B[DD3][1]+L)%L]>0)
            {
                d=A[(a+2*B[DD3][0]+L)%L][(b+2*B[DD3][1]+L)%L];
                fk=1+3*(d-1)-B[DD3][0];
                fk1=1-B[DD3][1];
                if(A2[fk][fk1]==2)
                {
            ...
            A[(a1+B[Dx2][0]+L)%L][(b1+B[Dx2][1]+L)%L]=-N-i-1;/*这里有问题，需要修改一下。以及先拿起来，再放下这个操作放在最外*/
            A[(a1+B[Dx3][0]+L)%L][(b1+B[Dx3][1]+L)%L]=-N-i-1;
            ...
            if(F2<F1)
            {
                ...
                A[(P2[i][0]+B[D2[i][2]][0]+L)%L][(P2[i][1]+B[D2[i][2]][1]+L)%L]=0;
                P2[i][0]=a,P2[i][1]=b;
                D2[i][0]=(D2[i][0]+Dk+6)%6;D2[i][1]=(D2[i][1]+Dk+6)%6;D2[i][2]=(D2[i][2]+Dk+6)%6;
                Etol=Etol+Eo-En;
                ...
                A[(P2[i][0]+B[D2[i][2]][0]+L)%L][(P2[i][1]+B[D2[i][2]][1]+L)%L]=-N-i-1;
                E3=E3+Eo;E4=E4+En;
                Insum11=Insum11+in11;
                Insum12=Insum12+in12;
            }
